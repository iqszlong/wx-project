<template>
  <view class="my-address-add">
    ...
    <div class="n-form-item">
      ...
      <nut-switch v-model="isDefault" :disable="isFrist" @change="defaultChange" />
    </div>
...
export default {
  ...
  setup() {
    ...
    const state = reactive({
      canSubmit: false,
      isFrist: routerData?.isFrist || false,// 是否是第一个添加的地址
    });
    ...
    onMounted(() => {
      getArea();
      if (state.isFrist) {
        data.isDefault = state.isFrist;
        form.default = state.isFrist;
      }
    })
